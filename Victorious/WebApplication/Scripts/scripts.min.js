...
jQuery(document).ready(function(){function e(){n("#TournamentCreate .bracketInfo").addClass("hide"),n("."+a[n("#TournamentCreate #BracketType").val()]).removeClass("hide")}var n=jQuery,a={3:"rr",6:"swiss"};n("#TournamentCreate #BracketType").on("change",e),function(n){1==n("#TournamentCreate").length&&e()}(n)});
jQuery(document).ready(function(){function e(){var e=s(this).data("id");e>-1&&s(".TournamentMatch .defender[data-id='"+e+"'], .TournamentMatch .challenger[data-id='"+e+"']").addClass("teamHover")}function a(){var e=s(this).data("id");e>-1&&s(".TournamentMatch .defender[data-id='"+e+"'], .TournamentMatch .challenger[data-id='"+e+"']").removeClass("teamHover")}function n(e,a){overview=a.find(".overview"),games=a.find(".TournamentGames"),overview.find(".defender .name").text(e.defender.name),overview.find(".defender .score").text(e.defender.score),overview.find(".defender").attr("data-id",e.defender.id).data("id",e.defender.id),overview.find(".challenger .name").text(e.challenger.name),overview.find(".challenger .score").text(e.challenger.score),overview.find(".challenger").attr("data-id",e.challenger.id).data("id",e.challenger.id),games.find(".defender.name").text(e.defender.name),games.find(".challenger.name").text(e.challenger.name),games.find(".games").empty(),s.each(e.games,function(e,a){t(a,games)}),e.ready?a.find(".details").removeClass("hide"):a.find(".details").addClass("hide")}function t(n,t){html="<ul class='form gameDetail' data-columns='4' data-gameid='"+n.id+"' data-gamenum='"+n.gameNum+"'> ",html+="<li class='column game-number'>"+n.gameNum+"</li> ",html+="<li class='column defender score' data-id='"+n.defender.id+"'><input type='text' class='defender-score' name='defender-score' maxlength='3' value='"+n.defender.score+"' /></li> ",html+="<li class='column challenger score' data-id='"+n.challenger.id+"'><input type='text' class='challenger-score' name='challenger-score' maxlength='3' value='"+n.challenger.score+"' /></li> ",html+="<li class='column'><span class='icon icon-bin removeGame'></span></li> ",html+="</ul> ",t.find(".games").append(html),s(".TournamentMatch .defender, .TournamentMatch .challenger").off("mouseover").on("mouseover",e),s(".TournamentMatch .defender, .TournamentMatch .challenger").off("mouseleave").on("mouseleave",a),s(".TournamentMatch .removeGame").off("click").on("click",r)}function d(e){return!(e.find(".games ul").length>=e.find(".games").data("max"))}function o(e,a){d(a)?a.find(".options .add-game").removeClass("hide"):a.find(".options .add-game").addClass("hide"),e.finished?(a.find(".update-games").addClass("hide"),a.find(".options .add-game").addClass("hide")):a.find(".update-games").removeClass("hide")}function r(){var e=s(this).closest("ul"),a={bracketId:e.closest(".bracket").data("id"),matchId:e.closest(".TournamentMatch").data("id"),matchNum:e.closest(".TournamentMatch").data("matchnum"),gameId:e.data("gameid"),gameNum:e.data("gamenum")};s.ajax({url:"/Ajax/Match/RemoveGame",type:"post",data:{jsonData:JSON.stringify(a)},dataType:"json",beforeSend:function(){},success:function(a){a=JSON.parse(a),console.log(a),a.status?(UpdateStandings(s("#Tournament").data("id"),e.closest(".bracket").data("bracketnum")),s.each(a.data,function(e,a){$match=s(".TournamentMatch[data-id='"+a.matchId+"']"),n(a,$match),o(a,$match),$games=$match.find(".TournamentGames"),$games.find(".games").empty(),s.each(a.games,function(e,a){t(a,$games)})})):console.log(a.message)},error:function(e){console.log(e)},complete:function(){}})}var s=jQuery;s(".TournamentMatch .defender, .TournamentMatch .challenger").on("mouseover",e),s(".TournamentMatch .defender, .TournamentMatch .challenger").on("mouseleave",a),s(".TournamentGames .removeGame").on("click",r),s(".TournamentGames .options .close").on("click",function(){s(this).closest(".TournamentGames").removeClass("open")}),s(".TournamentGames .options .add-game").on("click",function(){var e=s(this).closest(".TournamentGames"),a=(e.data("max"),e.find(".list-table-body ul").length),n=s(e.find(".games ul")[a-1]).data("gamenum")+1;jsonData={id:-1,gameNum:isNaN(n)?1:n,challenger:{score:""},defender:{score:""}},t(jsonData,e),d(e)?s(this).removeClass("hide"):s(this).addClass("hide")}),s(".TournamentMatch .details").on("click",function(){var e=s(this).closest(".TournamentMatch"),a={matchId:s(e).data("id")};s.ajax({url:"/Ajax/Match",type:"POST",data:{jsonData:JSON.stringify(a)},dataType:"json",beforeSend:function(){s(".TournamentInfo, .TournamentGames").removeClass("open"),e.find(".TournamentGames").addClass("open"),e.find(".TournamentGames .list-table-body").empty()},success:function(a){(a=JSON.parse(a)).status?(console.log(a),$games=e.find(".TournamentGames"),n(a.data,e),o(a.data,$games)):console.log(a.message)},error:function(a){a=JSON.parse(a),console.log(a),e.find(".TournamentGames").removeClass("open")},complete:function(){}})}),s(".TournamentGames .update-games").on("click",function(){var e=s(this).closest(".TournamentMatch"),a=e.find(".games ul"),t=new Array;s.each(a,function(e,a){defenderScore=s(a).find(".defender-score").val(),challengerScore=s(a).find(".challenger-score").val(),defenderScoreValid=s.isNumeric(defenderScore)||Math.floor(defenderScore)!=defenderScore,challengerScoreValue=s.isNumeric(challengerScore)||Math.floor(challengerScore)!=challengerScore,defenderScoreValid&&challengerScoreValue?s(a).find(".score").removeClass("invalid"):s(a).find(".score").addClass("invalid")});var d={matchId:e.data("id"),matchNum:e.data("matchnum"),bracketNum:s(this).closest(".bracket").data("bracketnum"),tournamentId:s("#Tournament").data("id")};s.each(a,function(e,a){t.push({GameNumber:s(a).data("gamenum"),DefenderScore:s(a).find(".defender-score").val(),ChallengerScore:s(a).find(".challenger-score").val()})}),s.ajax({url:"/Ajax/Match/Update",type:"POST",data:{jsonIds:JSON.stringify(d),games:t},dataType:"json",beforeSend:function(){e.find(".TournamentGames .update-games").attr("disabled",!0)},success:function(a){if((a=JSON.parse(a)).status){console.log(a.message);var t=null;if(a.data.refresh)return void window.location.reload();a.data.currentMatch.finished&&e.find(".TournamentGames").removeClass("open"),a.data.currentMatch&&(t=s(".TournamentMatch[data-id='"+a.data.currentMatch.matchId+"']"),n(a.data.currentMatch,t),o(a.data.currentMatch,t.find(".TournamentGames"))),a.data.winnerMatch&&(t=s(".TournamentMatch[data-id='"+a.data.winnerMatch.matchId+"']"),n(a.data.winnerMatch,t)),a.data.loserMatch&&(t=s(".TournamentMatch[data-id='"+a.data.loserMatch.matchId+"']"),n(a.data.loserMatch,t)),UpdateStandings(d.tournamentId,d.bracketNum)}else console.log("Error in updating");console.log(a)},error:function(e){console.log(e)},complete:function(){e.find(".TournamentGames .update-games").attr("disabled",!1)}})}),s(".TournamentGames .options .reset-match").on("click",function(){var e=s(this).closest(".TournamentMatch");s(this).closest(".bracket").data("id"),e.data("matchnum");s.ajax({url:"/Ajax/Bracket/MatchReset",type:"post",data:{bracketId:s(this).closest(".bracket").data("id"),matchNum:e.data("matchnum")},dataType:"json",success:function(a){(a=JSON.parse(a)).status&&(s.each(a.data,function(a,t){n(t,s(".TournamentMatch[data-id='"+t.matchId+"']")),t.matchNum==e.data("matchnum")&&o(t,e.find(".TournamentGames"))}),e.find(".TournamentGames .games").empty(),UpdateStandings(s("#Tournament").data("id"),e.closest(".bracket").data("id"))),console.log(a.message)},error:function(e){console.log(e)}})})});
